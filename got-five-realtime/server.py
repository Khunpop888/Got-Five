...
def log_public(item: dict[str, Any]) -> dict[str, Any]:
    return {key: value for key, value in item.items() if key != "actorAvatar"}


def chat_public(item: dict[str, Any]) -> dict[str, Any]:
    return {key: value for key, value in item.items() if key != "avatar"}
...
def apply_guess(room: Room, player: Player, guess: Any) -> dict[str, Any]:
    ...
    player.stats["guessRound"] = round_info(room)["current"]
...
def apply_match_to_series(room: Room) -> None:
    ...
    for rank in room.rankings:
        ...
        guess_round = player.stats.get("guessRound")
        if guess_round is not None:
            entry["guessRounds"].append(safe_int(guess_round, 0))
    ...
    for player in sorted(room.players, key=lambda item: item.seat):
        ...
        entry = {
            **base,
            ...
            "guessRounds": list(base["guessRounds"]),
        }
        ...
        entry["lastGuessRound"] = entry["guessRounds"][-1] if entry["guessRounds"] else None
        entry["avgGuessRound"] = (
            round(sum(entry["guessRounds"]) / len(entry["guessRounds"]), 1)
            if entry["guessRounds"]
            else None
        )
...
def summarize_match(room: Room) -> dict[str, Any]:
    ...
    return {
        ...
        "timeline": [log_public(item) for item in room.log[-120:]],
        "players": [
            {
                "id": player.id,
                "name": player.name,
...
def summarize_series(room: Room, include_history: bool = True) -> dict[str, Any]:
    return {
        ...
        "history": room.match_history[-5:] if include_history else [],
    }
...
def serialize_room(room: Room, viewer_player_id: str | None) -> dict[str, Any]:
    ...
    return {
        ...
        "log": [log_public(item) for item in room.log[-30:]],
        "chat": [chat_public(item) for item in room.chat[-50:]],
        "marks": sorted(viewer.marks) if viewer else [],
        "match": summarize_match(room) if room.status in {"finished", "between_matches"} else None,
        "series": summarize_series(room, include_history=room.status in {"finished", "between_matches"}),
